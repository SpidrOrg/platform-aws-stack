...
inputs = merge(
  ...
  {
    function_name                           = "dev-krny-moodys-yearly-tf"
    ...
    environment_variables                   = {secret_name = dependency.secret_name.outputs.secret_name,dynamodb_table = dependency.dynamodb_table.outputs.dynamodb_table_id, bucket = dependency.s3_bucket_id_external_sources.outputs.s3_bucket_id, date_column =	"yearly_date", file_name =	"moodys_yearly.csv", file_path	= "raw-data/moodys_all/data/yearly/", freq_code	= 204, gluejobname = "dev_krny_trnsf_moodys_tf", mapping_file_name = "moodys_all_mnemonics.csv", mapping_file_path = "raw-data/moodys_all/config/"  }
