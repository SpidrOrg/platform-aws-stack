...
inputs = merge(
  ...
  {
    function_name = "ingestion-moodys"
    s3_key        = "functions/moodys/lambda_function.py.zip"
    runtime       = "python3.8"
    ...
    environment_variables = { gluejobname = dependency.job_name.outputs.glue_job_name[0], secret_name = dependency.secret_name.outputs.secret_name, dynamodb_table = dependency.dynamodb_table.outputs.dynamodb_table_id, bucket = dependency.s3_bucket_id_external_sources.outputs.s3_bucket_id, date_column = "monthly_date", file_name = "moodys_monthly.csv", file_path = "raw-data/moodys/data/monthly/", freq_code = 128, mapping_file_name = "moodys_mnemonics.csv", mapping_file_path = "raw-data/moodys/config/" }
