...
inputs = merge(
  ...
  local.lambda_vars.inputs,
  {
    ...
    environment_variables                   = {   gluejobname = dependency.job_name.outputs.glue_job_name[0], file_path = "raw-data/fred/",bucket = dependency.s3_bucket_id_external_sources.outputs.s3_bucket_id , dynamodb_table = dependency.dynamodb_table_id.outputs.dynamodb_table_id } //secret = dependency.secret_name.outputs.secret_name,
