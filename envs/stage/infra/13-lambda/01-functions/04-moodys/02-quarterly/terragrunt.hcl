...
inputs = merge(
  ...
  local.lambda_vars.inputs,
  {
    ...
    environment_variables                   = {secret_name = "krny-moodys-secret",dynamodb_table = "krny-moodys", bucket = dependency.s3_bucket_id_external_sources.outputs.s3_bucket_id, date_column =	"quarterly_date", file_name =	"moodys_quarterly.csv", file_path	= "raw-data/moodys_all/data/quarterly/", freq_code	= 172, gluejobname = "transformation-moodys", mapping_file_name = "moodys_all_mnemonics.csv", mapping_file_path = "raw-data/moodys_all/config/"  }
